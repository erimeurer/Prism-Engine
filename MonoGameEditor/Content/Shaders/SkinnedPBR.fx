// Skinned PBR Effect - Physically Based Rendering with Skeletal Animation
// Based on PBR shader with added bone skinning support
...
// Vertex Shader Input (with skinning data)
...
// Vertex Shader with Skeletal Skinning
VertexShaderOutput MainVS(VertexShaderInput input)
{
    VertexShaderOutput output;
    
    // ===== SKELETAL SKINNING (Model Space -> World Space) =====
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (weight > 0.0)
        {
            // XNA/MonoGame convention: mul(Vector, Matrix) for row vectors
            float4 localPosition = mul(input.Position, Bones[boneIndex]);
            skinnedPosition += localPosition * weight;
            
            // Transform normal (3x3 part only)
            float3 localNormal = mul(input.Normal, (float3x3)Bones[boneIndex]);
    ...
    // ===== VIEW AND PROJECTION =====
    // skinnedPosition is now in World Space
    float4 viewPos = mul(skinnedPosition, View);
    output.Position = mul(viewPos, Projection);
    
    output.WorldPos = skinnedPosition.xyz;
...
/* 
// ORIGINAL SKINNING CODE - SAVED FOR LATER
VertexShaderOutput MainVS_Original(VertexShaderInput input)
{
    VertexShaderOutput output;
    ...
    float4 worldPos = mul(skinnedPosition, World); // Or just skinnedPosition if bones are already World
