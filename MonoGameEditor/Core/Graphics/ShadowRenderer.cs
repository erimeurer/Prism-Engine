...
namespace MonoGameEditor.Core.Graphics
{
    ...
    public class ShadowRenderer
    {
        ...
        private Effect _shadowDepthEffect = null!;
        ...
        private void LoadEffect(Microsoft.Xna.Framework.Content.ContentManager content)
        {
            try
            {
                ...
                Logger.Log("[ShadowRenderer] ShadowDepth shader loaded with techniques: " + 
                    string.Join(", ", _shadowDepthEffect.Techniques.Select(t => t.Name)));
            }
            catch (System.Exception ex) 
            { 
                 Logger.Log($"[ShadowRenderer] Failed to load ShadowDepth: {ex.Message}");
            }
        }
